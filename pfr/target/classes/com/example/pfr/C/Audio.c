...
int indexerAudio(char* cheminFichier,char* nomFichier, char* descripteur){
    int nbIntervalle;
    int echantillonage;
    system("cat config.txt | grep nombrePartition | cut -d' ' -f2 > partitionTemp.txt");
    FILE * fichier = NULL;
    fichier = fopen("partitionTemp.txt", "r");
    fscanf(fichier, "%d", &nbIntervalle);
    fclose(fichier);
    system("rm partitionTemp.txt");
    system("cat config.txt | grep nombreEchantillonage | cut -d' ' -f2 > echantTemp.txt");
...
int comparerAudio(char* cheminFichier, char* nomFichier,char* descripteurs, FICHIER* listeFichiers,int nbIntervalle, int echantillonage){
    //création d'un descripteur temporaire qui contiendra l'indexation du fichier recherché
    FILE* descRecherche = fopen("descFichierRecherche.txt","w+");
    //on teste l'ouverture du fichier
    if(descRecherche==NULL){
        return 1;
    }
    //on indexe le fichier et ainsi initialise le descripteur temporaire
    indexerAudio(cheminFichier,nomFichier,"descFichierRecherche.txt");
    //on ferme le foichier temporaire
    fclose(descRecherche);

    //ouverture du fichier contenant tous les descripteurs
    FILE* tempDescripteurs =NULL;
    tempDescripteurs = fopen(descripteurs,"r");
    //on teste l'ouverture du fichier
    if(tempDescripteurs==NULL){
        return 1;
    }
    //lecture du descripteur en le comparant avec les valeur du fichier resCMD1.txt
    FILE* tempFichierRecherche = fopen("descFichierRecherche.txt","r");
    //on teste l'ouverture du fichier
    if(tempFichierRecherche==NULL){
        return 1;
    }
    char valInRes='p';
    char valInDesc='p';
    ...
    int j=0;
    int debut=0;
    //boucle permettant d'effectuer des lectures de valeurs tant que la fin de "descripteurs" n'es pas atteinte
    while(!strchr("NESC", valInDesc)){
        //si c'est le début de la lecture du descripteur
        if(debut==0){
            //on passe la première ligne du fichier
            while(valInDesc!='\n'){
                fread((void *)&valInDesc, sizeof(valInDesc), 1, tempDescripteurs);
            }       
            int l = 0;
            fread((void *)&valInDesc, sizeof(valInDesc), 1, tempDescripteurs);
            //on passe à la ligne concernant le titre
            while(valInDesc!='\n'){
                listeFichiers[j].nomFichier[l]=valInDesc;
                fread((void *)&valInDesc, sizeof(valInDesc), 1, tempDescripteurs);
                l++;
            }
            //nous ne rencontrerons plus la première ligne du descripteur
            debut=1;
        } 
        //Lecture des valeurs récupérées (1 bloc à la fois) dans "descripteurs.txt"
        //Stockage dans la variable "valInDesc"
        fread((void *)&valInDesc, sizeof(valInDesc), 1, tempDescripteurs);
        //Lecture des valeurs récupérées (1 bloc à la fois) dans "resCMD1.txt"
        //Stockage dans la variable "valInRes"
        fread((void *)&valInRes, sizeof(valInRes), 1, tempFichierRecherche);
        //Si on arrive à la fin d'un descripteur contenue dans "descripteurs.txt"
        if(valInDesc=='d'){
            //Lecture des valeurs récupérées (1 bloc à la fois) dans "descripteurs.txt"
            //Stockage dans la variable "testFin"
            fread((void *)&testFin, sizeof(testFin), 1, tempDescripteurs);
            if(testFin=='D'){
                //on repars de 0 dans la lecture du fichier descFichierRecherche.txt
                rewind(tempFichierRecherche);
                //on incrémente l'itérateur j
                //Ainsi on passe au fichier suivant dans listeFichiers
                j++;
                if(debut==1){
                    //on passe la première ligne du fichier
                    while(valInDesc!='\n'&&!strchr("DNESC", valInDesc)){
                        fread((void *)&valInDesc, sizeof(valInDesc), 1, tempDescripteurs);
                    }       
                    int l = 0;
                    fread((void *)&valInDesc, sizeof(valInDesc), 1, tempDescripteurs);
        ...
        else if(valInDesc==valInRes){
            listeFichiers[j].tauxDeSimilarite+=1;
        }
    }
    //fermeture des fichiers temporaires
    fclose(tempDescripteurs);
    fclose(tempFichierRecherche);
    triListeFichiers(listeFichiers);
    system("rm descFichierRecherche.txt");
    return 0;
}

/*
triListeFichiers: cette méthode tri la liste de FICHIER f en fonction des valeurs du taux de similarités
Paramètres:
    f: FICHIER*: pointe vers une liste de FICHIER
Retour:
    pas de retour : void
*/
void triListeFichiers(FICHIER* f){
    ...
    while(i!=10)
    {
        ...
        if (f[i].tauxDeSimilarite < f[i+1].tauxDeSimilarite) 
        {
            tmp = f[i].tauxDeSimilarite;
            f[i].tauxDeSimilarite = f[i+1].tauxDeSimilarite;
            f[i+1].tauxDeSimilarite = tmp;
        }
        i++;
    }
}